 ...
 */
public class CtrlProdutor implements Retorno {
    private Context contexto;

    public CtrlProdutor(Context contexto) {
    ...
    public void trazer(int codigo, final CallbackTrazer callback) {
        ...
        params.put("acao", "R");
        params.put("tabela", "produdor");
        params.put("condicao", "codigo");
        params.put("valores", String.valueOf(codigo));

        GetData<Produtor> getData = new GetData<>("objeto", params);
        getData.executar(Produtor.class, new CallBackDAO() {
            @Override
